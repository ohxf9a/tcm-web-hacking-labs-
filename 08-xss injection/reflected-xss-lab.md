...
Lab Link: Reflected XSS into HTML context with most tags and attributes blocked
...
Figure 2: Burp Intruder results showing which HTML tags are allowed (200) vs blocked (400)
...
3 Determine Filter Behavior Server returns 400 for blocked tags, 200 for allowed ones

Tag Discovery Process

Step Action Payload/Command

1 Use XSS Cheat Sheet Load PortSwigger's XSS cheat sheet for comprehensive tag list

2 Send to Intruder Configure Burp Intruder with the tag list as payloads

3 Analyze Results Filter responses by status code to identify allowed tags

4 Identify Allowed Tag <body> returns 200 OK while most others return 400
...
3 Create Exploit Page Use iframe to load vulnerable page with payload
...
Filter Bypass: The server's blacklist doesn't include <body> or onresize
...
print() Function Executes JavaScript, confirming XSS
...
onresize SUCCESS - Can be triggered by iframe resize
...
Content Security Policy Not implemented in this lab
...
1 Identify reflection point Complete

2 Determine allowed tags Complete

3 Find working event handler Complete

4 Craft XSS payload Complete

5 Build exploit chain Complete

6 Deliver to victim Complete

7 Confirm execution Complete

Final Exploit Payload

html

<iframe src="https://0a6000eb042610c080b4a34b00e300b9.web-security-academy.net/?search=%3Cbody+onresize%3Dprint%28%29%3E" onload=this.style.width="1em">
...
src=".../?search=%3Cbody+onresize%3Dprint%28%29%3E" Loads vulnerable page with XSS payload

onload=this.style.width="1em" Triggers resize of iframe, activating onresize

Key Takeaways

Blacklisting is ineffective - Blocking common tags/attributes isn't enough; proper output encoding is essential



Test all tags and attributes - You might find unexpected vectors like <body> with onresize



Consider triggering mechanisms - Some event handlers require specific interactions; find ones you can trigger programmatically



Browser differences matter - The victim uses Chrome; some events work differently across browsers



Iframes are powerful exploitation tools - They can be used to trigger events on other pages



Event handlers can be triggered by CSS/JS - Even simple operations like resizing can execute JavaScript



Author

Water | Date: 2026-07-25 | GitHub: @ohxf9a

change this to markdwon format svg i want to save it as .md file so use those ## and symbols and place links for the images
...
│
▼
[3. Find Event Handlers] ──> Test allowed tags (e.g., )
│
▼
...
```html
<iframe src="[https://0a6000eb042610c080b4a34b00e300b9.web-security-academy.net/?search=%3Cbody+onresize%3Dprint%28%29%3E](https://0a6000eb042610c080b4a34b00e300b9.web-security-academy.net/?search=%3Cbody+onresize%3Dprint%28%29%3E)" onload=this.style.width="1em"></iframe>
Payload Breakdown
Component	Purpose
<iframe ...>	Container to load the vulnerable page
...
Key Takeaways
Blacklisting is ineffective: Blocking common tags/attributes isn't enough; proper context-aware output encoding is essential.
...
Iframes are powerful exploitation tools: They can be used to trigger window/element events on embedded pages.

Event handlers can be triggered by CSS/JS: Even simple operations like dynamic resizing can trigger JavaScript execution.

Author: Water | Date: 2026-07-25 | GitHub: @ohxf9a
"""

filename = "PortSwigger_Reflected_XSS_Lab_Writeup.md"
...
File created successfully: PortSwigger_Reflected_XSS_Lab_Writeup.md

Your Markdown document is ready:
MD icon
PortSwigger_Reflected_XSS_Lab_Writeup
 MD 
Open

Summary of Formatting & Enhancements Applied
Markdown Hierarchy (#, ##, ###): Formatted headers with consistent structure.

Image Embeds: Added Markdown image syntax (![Figure Name](path)) and descriptive captions for each figure.

Tables & Checklists: Converted methodology steps, core concepts, payload breakdowns, and progression milestones into clean Markdown tables and task lists (- [x]).

Code Blocks: Syntax-highlighted HTML payload examples (```html) and diagram blocks.
